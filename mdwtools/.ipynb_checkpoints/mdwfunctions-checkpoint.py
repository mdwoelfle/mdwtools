...
def calcltsda(ds):
    """
    Create LTS dataArray from ds (on plevs)

    Notes:
        LTS = THETA_700 - THETA_1000
        THETA_X = T_X*(P_0/P_X)^(R/c_p)
        where P_0 = 
    """
    ...
    # Set gas constant ration (kappa = R/c_p)
    kappa = 2./7.  # approximation
    
    # Compute Theta at 700 hPa
    theta700 = ds['T'].sel(plev=700)*(1000./700.)**kappa
    
    # Compute Theta at 1000 hPa
    theta1000 = ds['T'].sel(plev=1000)*(1000./700.)**kappa
    
    # Compute LTS
    ltsDa = xr.DataArray(theta700 - theta1000,
                        attrs=theta700.attrs
                        )
    
    # update long_name for LTS
    ltsDa.attrs['long_name'] = 'Lower Tropospheric Stability'
...
def calcdssepsstindex(ds,
                      indexType='tropicalRelative',
                      indexVar='TS',
                      ocnOnly_flag=True,  # Needed for obs
                      qc_flag=False,
                      ):
    # Default is as a difference from the tropics (-20 to 20)
    if indexType.lower() in ['tropicalrelative']:

        # Ensure land mask is available if needed
        if all([ocnOnly_flag,
                'LANDFRAC' not in ds]):
            raise ValueError('Need LANDFRAC to limit SST to ' +
                             'ocean grid points only')

        # Compute tropics mean SST
        tropRegMeanDa = calcdaregmean(ds[indexVar],
                                      gwDa=(ds['gw']
                                            if 'gw' in ds
                                            else None),
                                      latLim=np.array([-20, 20]),
        ...
        # Compute southeast Pacific tropics mean SST
        sepRegMeanDa = calcdaregmean(ds[indexVar],
                                     gwDa=(ds['gw']
                                           if 'gw' in ds
                                           else None),
                                     latLim=np.array([-20, 0]),
        ...
        # Compute relative SEP SST
        sepIndexDa = (sepRegMeanDa - tropRegMeanDa)

        # Make plot for QC purposes
        if qc_flag:
            plt.figure()
            plt.plot(sepRegMeanDa.values, label='SEP')
            plt.plot(tropRegMeanDa.values, label='Tropics')
            plt.legend()

        # Update attributes on paiDa
        sepIndexDa.name = 'SEPIndex'
        sepIndexDa.attrs['long_name'] = ('SEP SST Index (SEP - tropics)')
        sepIndexDa.attrs['units'] = ds[indexVar].units
    elif indexType.lower() in ['raw']:
        # Option to plot as raw values
        # Ensure land mask is available if needed
        if all([ocnOnly_flag,
                'LANDFRAC' not in ds]):
            raise ValueError('Need LANDFRAC to limit SST to ' +
        ...
        # Compute southeast Pacific SST
        sepIndexDa = calcdaregmean(ds[indexVar],
                                   gwDa=(ds['gw']
                                         if 'gw' in ds
                                         else None),
                                   latLim=np.array([-20, 0]),
        ...
        if qc_flag:
            plt.figure()
            plt.plot(sepIndexDa.values, label='SEP')
            plt.legend()

        # Update attributes on paiDa
        sepIndexDa.name = 'SEPIndex'
        sepIndexDa.attrs['long_name'] = ('Raw SEP SST Index (SEP only)')
        sepIndexDa.attrs['units'] = ds[indexVar].units
    else:
        raise ValueError('Unknown indexType, {:s}, '.format(indexType) +
                         'for computing SEP SST index.')
    return sepIndexDa
...
def convertsigmatopresds(dsIn,
    ...
    for jVar, regridVar in enumerate(regridVars):
        ...
        try:
            ...
            # Will get to this when regridding a sinlge variable as
            #    isobaric_levels will then be a np array rather than a list
            daOut = xr.DataArray(
                isobaric_levels,
                coords=[dsIn['time'],
                        newlevs/100.,  # Convert Pa to hPa
